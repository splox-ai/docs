...
<CardGroup cols={2}>
  ...
  <Card title="Memory Edges" icon="comments" href="/edges/memory-edges">
    **Stateful conversations**
    ...
    Connect memory nodes to LLMs for chat history
  </Card>
...
<Tabs>
  ...
  <Tab title="Output Handles">
    **Right side of nodes**
    ...
    - **TOOLS:** Tool calling path (LLM nodes)
    - **IF/ELIF/ELSE:** Conditional branches (Switch nodes)
  </Tab>
  
  <Tab title="Special Handles">
    **Bottom/top handles**
    
    - **MEMORY (bottom):** On LLM nodes for memory connection
    - **MEMORY (top):** On Memory nodes to connect upward
    - Used exclusively for memory relationships
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="AI Agent with Tools">
    **LLM with multiple tools in a loop**
    ...
    Use tool edges in a subflow for agent patterns.
  </Tab>
...
<CardGroup cols={2}>
  ...
  <Card title="Build a Workflow" icon="hammer" href="/guides/first-workflow">
    Create your first workflow with edges
  </Card>
  
  <Card title="Testing" icon="flask" href="/guides/testing-workflows">
    Test and debug edge connections
  </Card>
</CardGroup>
