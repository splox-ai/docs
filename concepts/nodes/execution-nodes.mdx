...
description: "Control workflow entry, exit, and termination with Entry, End, and Stop nodes"
---
...
<CardGroup cols={3}>
  <Card title="Entry Node" icon="play">
...
Every workflow must have at least one entry node. When a workflow is triggered, execution begins at the specified entry node(s).
...
**Features:**
- Multiple entry nodes supported for multi-entry workflows
...
<Tip>
  Entry nodes can be selected when triggering a workflow programmatically, allowing different entry points for different use cases.
</Tip>
...
<Tabs>
  ...
  <Tab title="Output Handle">
    **Right Side - Loop Output (Subflows Only)**
    ...
    - **LOOP:** Connects back to entry node for iteration
...
- **End Node**: Completes the current iteration. If more iterations remain, execution loops back to Entry Node.
