...
<CardGroup cols={3}>
  ...
  <Card title="Configurable Limits" icon="gauge">
    Set max iterations, stop conditions, and timeouts to control behavior
  </Card>
...
<Steps>
  ...
  <Step title="Iteration">
    ...
    - A **stop condition** template evaluates to true
  </Step>
...
<AccordionGroup>
  <Accordion title="Provider & Model" icon="server">
    ...
    |-------|-------------|---------|
    | **Text LLM Provider** | Provider slug (`anthropic`, `openai`, `gemini`, `openrouter`, `perplexity`, etc.) | `anthropic` |
  ...
  <Accordion title="System Prompt" icon="message">
    The system prompt instructs the agent on its role, behavior, and constraints. It supports **Pongo2 templates** for dynamic content.
...
|-------|-------------|---------|-------|
| **Max Iterations** | Maximum number of LLM → Tool → LLM cycles | `10` | 1–50 |
| **Stop Condition** | Pongo2 template that evaluates to true/false for early exit | None | — |
| **Timeout** | Maximum execution time in minutes | 525,600 (1 year) | — |
| **Max Retries** | Retries on transient failures | `3` | 0–10 |

<Warning>
  Setting max iterations too high can lead to excessive credit usage. Start with 10 and increase as needed.
</Warning>
