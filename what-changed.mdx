...
| Entry node | `handle(msg)` in `programs/splox/main.py` | [Programs](/build/programs) |
| Tool node | a Python function in `tools/`, imported and called in code | [Tools](/build/tools) |
| Switch node, conditional edge | `if` | [Programs](/build/programs) |
| Parallel edges | `parallel(...)` and `Pool` over sub-agent runs | [Sub-agents](/build/subagents) |
| Merge node | the code after those calls return | [Sub-agents](/build/subagents) |
| Error edge | `hooks/errors.py` — `on(t, err)` answers a provider that failed | [Hooks](/build/hooks) |
| Tool edge | `tools=[…]` on the agent; `hooks/tools.py` sees every call | [Hooks](/build/hooks) |
| Variable mappings | ordinary Python variables, and the prompt you hand a sub-agent | [Sub-agents](/build/subagents) |
| Node lifecycle | the turn loop and its ten hook points | [Hooks](/build/hooks) |
| Workflow version | a harness version: `refs/versions/N`, published by pushing | [Versions](/build/versions) |
| Execution tree | the run tree: `GET /v2/runs/{run_id}/tree` | [Runs](/api/runs) |
| Run a workflow | `POST /v2/runs` | [Runs](/api/runs) |
| Trigger webhook | a program in the harness that listens, started on the machine | [Patterns](/build/patterns) |
| Memory instance | files on the machine, and the window's own compaction | [Memory](/build/memory) |
| MCP connection | **Connections**; a connected server shows up as a tool package | [Tools](/build/tools) |
...
<Steps>
  ...
  <Step title="Port the one workflow you actually miss">
    Write it as a program. [Patterns](/build/patterns) has four shapes to copy: a
